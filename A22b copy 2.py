...
def find_segment_intersections(segments):
    ...
    for i, segment in enumerate(segments):
        start, end = segment
        events.append(Event(start[0], i, "start"))
        events.append(Event(end[0], i, "end"))
...
if __name__ == "__main__":
    ...
    num_segments = 100  # valor máximo de segmentos
    segment_length = 4
    l = []
    t = []
    
    for n in range(1, num_segments, 10):
        line_segments = generate_random_line_segment(segment_length, n)
        start = time.time()
        _ = [find_segment_intersections(line_segments) for _ in range(m)]
        end = time.time()
        dur = end - start
    ...
    plt.title('Segment Intersection Detection Time')
    plt.grid()
    plt.show()
